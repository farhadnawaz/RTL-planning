...
class ustar_m:
    ...
    def compute_ustar_m(self, count, state_data, NN_vel, control_t, control_t_p, change_flag, trans_period, beta_t, k, t_star, 
        ...
        dist_ref = jnp.linalg.norm(xref - x_t.reshape((1,-1)), axis=1)
        ...
        ref_range_start = jnp.where(control_t == 3, 5, self.ref_range_start) # look ahead horizon for wiping stain and nominal target
        ...
        xref_t = xref[xref_ind].reshape((-1,1))
...
def main():
    ...
    change_flag = 0
    v_uniform = 0.2
    target_array = jnp.zeros((200, 3))
    vel_array =jnp.zeros((200, 3))
    ...
    rospy.Subscriber('/my_aut_plan/dirt_pos', Vector3, dirt_pos_callback,
                                    queue_size=1, tcp_nodelay=True)
    rospy.Subscriber('/my_aut_plan/grip_pos', Vector3, dip_pos_callback,
                                    queue_size=1, tcp_nodelay=True)
