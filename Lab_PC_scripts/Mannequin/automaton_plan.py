...
from std_msgs.msg import Int32, Int32MultiArray, Float64MultiArray, Bool
...
dip_pos_global = jnp.array([0.67, -0.35, 0.24]).reshape((-1,1))
...
def human_callback(human_msg):
    global human_data
    human_data = jnp.array(human_msg.data)
...
def main():
    global  buttons_data, dip_pos_global, human_data
    ...
    first_dip = 0
    human_out = 0 # initially human out for dipping behavior
    ...
    while not rospy.is_shutdown():
        ...
        if stain_count == 200 and control_prop_t<=1:
            ...
            increase_z = 0.015 # for real robot, 0 for gazebo (too small a velocity near dirt_pos in real robot)
            aut_dec_obj.dirt_pos = jnp.copy(dirt_pos) + jnp.array([0.0, 0.0, increase_z]).reshape((3,1))
        ...
        if control_prop_t == 4:
            if jnp.linalg.norm(aut_dec_obj.xt - aut_dec_obj.dip_pos)<(epsilon_B + 0.06):
                if first_dip == 0:
                    first_dip = 1
                    aut_dec_obj.dip_pos = jnp.copy(dip_pos_global) + jnp.array([0.0, 0.0, -0.15]).reshape((-1,1))
                elif first_dip == 1:
                    time.sleep(5) 
                    first_dip = 2
                    aut_dec_obj.dip_pos = jnp.copy(dip_pos_global) + jnp.array([0.0, 0.0, 0.1]).reshape((-1,1))
                elif first_dip == 2:
                    if human_data == 0 and human_out == 0:
                        human_out = 1
                        time.sleep(5) # wait for human to come in
                    elif human_data == 1 and human_out == 1:
                        time.sleep(10) # wait for human to squeeze the towel
                    elif human_data == 0 and human_out == 1:
                        uncontrol_AP[2] = 1
