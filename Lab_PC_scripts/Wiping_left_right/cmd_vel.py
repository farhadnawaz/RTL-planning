...
class ustar_m:
    ...
    def __init__(self, ref_range_start, ref_range_start_wiping, ref_range, vopt_max, 
                 v_uniform, xt, fxt, control_t, control_t_p, dirt_pos, 
                 trans_period, beta_t, dt, dt_beta_t, change_flag):
        self.ref_range_start = ref_range_start # from where to start looking forward?
        self.ref_range_start_wiping = ref_range_start_wiping
    ...
    def target_track(self, x1, xref_t, fx1, fxref_t, pos,
        ...
        z_orig = xref_t[2, 0]        
        xref_t = xref_t.at[2, 0].set(z_orig - 0.02)
    ...
    # @partial(jit, static_argnums=(0,))
    @jit
    def dirt_wiping(self, x1, xref_t, fx1, fxref_t, pos,
                     STL_t, t_star, k, scaler_t):
        ...
        fx1_0 = fx1[:, 0]
        fxref_t_0 = fxref_t[:, 0]
        # G = jnp.vstack((dV_de.T, jnp.eye(fx1_0.shape[0]), -jnp.eye(fx1_0.shape[0])))
        h = jnp.array([-alpha_BB_CLF  + jnp.sum(dV_de * (fxref_t_0))])
        # h = jnp.hstack((jnp.array([-alpha_BB_CLF  - jnp.sum(dV_de * (fx1_0 - fxref_t_0))]),
        #                 vopt_bound, vopt_bound))
        # # rospy.loginfo(G.shape)
        ...
        vel_cmd = k_scale*(ustar)
        # vel_cmd = k_scale*self.scaler_t_wiping*(ustar + fx1)

        return jnp.float32(vel_cmd)
    ...
    # @partial(jit, static_argnums=(0,))
    @jit
    def compute_ustar_m(self, count, state_data, NN_vel, control_t, control_t_p, change_flag, trans_period, beta_t, k, t_star, 
                        STL_t, vel_cmd_prevs, dirt_pos, dip_pos, u1, xref_DS, xref_vel_DS, scaler_t, target_wiping_array, vel_wiping_array):
        ...
        xref_wiping = target_wiping_array
        # xref = jnp.where(control_t == 3, xref_wiping, xref_DS[:, :dim])
        xref_vel_wiping = vel_wiping_array
        # xref_wiping = xref
        ...
        dist_ref_w = jnp.linalg.norm(xref_wiping - x_t.reshape((1,-1)), axis=1)
        ...
        loop_ind = jnp.where(control_t == 3, terminal_time-1, 0)
        ...
        pos = jnp.where(control_t == 2, dirt_pos, dip_pos)
        
        STL_t = jnp.where(control_t < 2, STL_t, STL_t + self.dt)
        ...
        branches = [self.target_track, self.target_track, self.go_to_dirt, self.dirt_wiping, self.go_to_dip]
        ...
        vel_cmd_robot_p = jnp.where(count<2, jnp.zeros(vel_cmd_robot.shape), vel_cmd_robot)
    ...
    def _tree_flatten(self):
        ...
        return (self.ref_range_start, self.ref_range_start_wiping, self.ref_range, self.vopt_max, self.v_uniform, self.xt, self.fxt, self.control_t, self.control_t_p, self.dirt_pos, self.trans_period, 
                self.beta_t, self.dt, self.dt_beta_t, self.change_flag,), ()
...
def main():
    ...
    change_flag = 0
    v_uniform = 0.2
    target_array = jnp.zeros((100, 3))
    vel_array =jnp.zeros((100, 3))
    ...
    control_t_ind = jnp.where(control_t > 1, 0, control_t)
    ...
    ustar_obj = ustar_m(ref_range_start, ref_range_start_wiping, ref_range, vopt_max, v_uniform, 
                        xt, fxt, control_t, control_t_p, dirt_pos, trans_period, beta_t, dt, dt_beta_t, change_flag)
    ...
    while not rospy.is_shutdown():
        ...
        if control_t == 2 and change_flag:
            data_file = curr_path + '/config/Trajectory_data_eff/trajectory_hand_dirt.npy'
        ...
        f1x, f1y, f1z, trans_period, beta_t, change_flag, k, t_star, STL_t, vel_cmd_prevs, u1 = ustar_obj.compute_ustar_m(count, xt, fxt, control_t, control_t_p, change_flag, trans_period, beta_t, k, t_star, STL_t, vel_cmd_prevs, dirt_pos, dip_pos,
                                                                                                                          u1, xref, xref_vel, scaler_t, target_array, vel_array)
